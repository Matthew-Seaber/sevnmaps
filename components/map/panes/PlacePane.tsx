...
async function PlacePane({ placeID }: { placeID: string }) {
  async function fetchFullPaneData(): Promise<{
    place: Place;
    lists: List[];
  } | null> {
    if (!placeID) {
    ...
    try {
      ...
      if (!session) {
        throw new Error(
          "User is not authenticated to fetch this place's details.",
        );
      }
      ...
      const result = db.transaction(async (tx) => {
        ...
        const reviewImages = await tx
          ...
          .innerJoin(
            place_images,
            eq(review_image_link.imageId, place_images.id),
          )
  ...
  const data = await fetchFullPaneData();

  if (!data) {
    return <p>Error fetching place details. Please try again later.</p>;
  }

  return <h1>Place pane</h1>;
}
